...
- Run the app (no path config needed): `cd /workspace && export PATH="$PWD/node_modules/.bin:$PATH" && pi --no-session`. On first launch, choose "Trust" at the project-folder prompt. The startup dashboard should list 11 extensions (agent-todos, chat-mode, context7, cursor-models, model-manager, pi-lens, search-subagent, startup-dashboard, subagent-console, tapd, titlebar-working). The toolkit is already registered in Pi user settings via `pi install .`; if it isn't loading, re-run `pi install .` from the repo root.
