...
export const DESIGN_TRIGGER_PROMPT = [
	...
	"5. 完成需求和代码调研后、写入或覆盖 design.md 前，识别会实质影响范围、架构、兼容性、接口契约或验收标准的待确认决策。每个决策必须单独调用 ask_user_choice：提供 2～5 个具体选项，说明关键取舍，最多标记一个推荐项；工具会自动提供最后一项自定义输入。已经能从需求和代码确认的内容不要重复询问；确实没有待确认决策时可以跳过提问。",
	...
	"10. 将完整方案写入 understanding.md 同目录下的 design.md。",
...
export const COLLABORATION_TRIGGER_PROMPT = [
	...
	"13. 不要描述没有变化的 loading、权限、防重复提交等通用行为；“保持现状”只在容易误解时提一次。不要包含验收场景、待评审问题、排期、负责人或上线计划；不要修改代码。",
