...
import type { ItemKey, SessionLink, TapdItemKind, TapdLinkRecord } from "./types.js";
...
export function loadLinks(): Record<string, TapdLinkRecord> {
  try {
    if (existsSync(LINKS_PATH)) return JSON.parse(readFileSync(LINKS_PATH, "utf-8"));
  } catch {}
  return {};
}

export function saveLinks(links: Record<string, TapdLinkRecord>): void {
  try { writeFileSync(LINKS_PATH, JSON.stringify(links, null, 2), "utf-8"); } catch {}
}

export function linkKey(workspaceId: string, itemId: string, kind: TapdItemKind = "story"): string {
...
export function parseItemKey(key: string): ItemKey {
  ...
  return { kind: "story", wsId: parts[0] ?? "", itemId: parts.slice(1).join("_") };
}
...
export function getOrCreateLink(
  ...
  if (!links[key]) links[key] = { workspaceId, storyId: itemId, itemId, kind, name, sessions: [] };
  return links[key];
}
...
export function getTapdDocPath(cwd: string, itemId: string, fileName: string): string {
...
export function loadPathHistory(): string[] {
  try {
    ...
    return raw.filter((path): path is string => typeof path === "string" && path.trim().length > 0);
  } catch {
    return [];
  }
}
...
export function rememberProjectPaths(paths: string[]): void {
  ...
  try { writeFileSync(PATHS_HISTORY_PATH, JSON.stringify([...cleaned, ...history].slice(0, MAX_PATH_HISTORY), null, 2), "utf-8"); } catch {}
}

export function removeProjectPathFromHistory(path: string): void {
  const history = loadPathHistory().filter((item) => item !== path);
  try { writeFileSync(PATHS_HISTORY_PATH, JSON.stringify(history, null, 2), "utf-8"); } catch {}
}
...
): { links: Record<string, TapdLinkRecord>; key: string; record: TapdLinkRecord; session: SessionLink } | null {
  if (!sessionFile) return null;
  for (const [key, record] of Object.entries(links)) {
    const session = record.sessions.find((item) => item.sessionFile === sessionFile);
