...
import {
  buildTree, collectTypes, flatFilter, fmtDate, getTypeIcon, oneLine,
  padR, prioritySymbol, searchFlat, sortTree, tapdUrl,
} from "./model.js";
import {
  linkKey, loadLinks, loadPathHistory, parseItemKey, readSessionTitle,
  rememberProjectPaths, removeProjectPathFromHistory, saveLinks,
} from "./storage.js";
import type {
  PickerAction, SessionLink, TableOutcome, TapdConfig, TapdItem,
  TapdItemKind, TapdLinkRecord,
} from "./types.js";
...
class TreeList {
  ...
  render(width: number, theme: any): string[] {
    ...
    for (let i = start; i < end; i++) {
      ...
      const titleW = Math.max(1, maxW - visibleWidth(indent + marker + icon) - columnW - separatorW);
      let line = indent + marker + icon;
      line += " " + padR(truncateToWidth(oneLine(item.name), titleW, "…"), titleW);
      line += " " + padR(truncateToWidth(oneLine(item.status), statusW, ""), statusW);
      line += " " + padR(truncateToWidth(prioritySymbol(item.priority), priorityW, ""), priorityW);
      if (item.kind === "bug") line += " " + padR(truncateToWidth(oneLine(item.severity ?? "-"), severityW, ""), severityW);
...
export async function showTable(ctx: ExtensionCommandContext, c: TapdConfig, workspaces: { id: string; name: string }[], initialCurrent: boolean): Promise<TableOutcome> {
  const controller = new AbortController();
  const trees: Record<TapdItemKind, { current: TapdItem[]; all: TapdItem[]; allLoaded: boolean }> = {
  ...
  async function load(kindToLoad: TapdItemKind, scope: "current" | "all") {
    ...
    ctx.ui.notify(`正在获取${kindToLoad === "bug" ? "Bug" : "需求"}${scope === "current" ? "当前迭代" : "所有迭代"}待办...`, "info");
    const result = await fetchAll(workspaces, c, scope, controller.signal, kindToLoad);
    const data = kindToLoad === "story" ? buildTree(result.items) : result.items;
    sortTree(data);
    if (scope === "current") state.current = data;
    else { state.all = data; state.allLoaded = true; }
    if (result.errors.length > 0) ctx.ui.notify(`部分工作空间获取失败: ${result.errors.join(", ")}`, "warning");
  }
  ...
  while (true) {
    ...
    const display = kind === "story" && typeFilter ? flatFilter(source, typeFilter) : source;
    const viewLabel = viewCurrent ? "当前迭代" : "所有迭代";
    const sel = await renderTable(ctx, display, viewLabel, typeFilter, kind, state.current.length, trees.story.current.length, trees.bug.current.length);
    if (!sel) break;
    ...
    if (sel.action === "link_view" && sel.itemKey) {
      ...
      const rec = links[sel.itemKey] ?? (parsed.kind === "story" ? links[legacyKey] : undefined) ?? { workspaceId: parsed.wsId, storyId: parsed.itemId, name: sel.itemName!, sessions: [], kind: parsed.kind, itemId: parsed.itemId };
      const action = await showSessionPicker(ctx, rec, sel.itemKey, sel.itemName!);
      if (action) return { kind: "session_action", action, itemKey: sel.itemKey, itemName: sel.itemName! };
      continue;
    }
    break;
  }
...
async function renderTable(_ctx: ExtensionContext, forest: TapdItem[], viewLabel: string, typeFilter: string | null, kind: TapdItemKind, _currentCount: number, storyCount: number, bugCount: number): Promise<{ action: string; url?: string; itemKey?: string; itemName?: string } | null> {
  ...
  return await _ctx.ui.custom<{ action: string; url?: string; itemKey?: string; itemName?: string } | null>((tui, theme, _kb, done) => {
    ...
    function rebuildAll() {
      ...
      container.addChild(new Text(
        (kind === "story" ? theme.bg("selectedBg", theme.fg("text", ` 🎯 需求 ${storyCount} `)) : theme.fg("muted", ` 🎯 需求 ${storyCount} `))
          + " " + (kind === "bug" ? theme.bg("selectedBg", theme.fg("text", ` 🐛 Bug ${bugCount} `)) : theme.fg("muted", ` 🐛 Bug ${bugCount} `))
          + theme.fg("dim", `  │  ${viewLabel}  ${shownCount}${searching ? "/" + total : ""} 项`)
      ...
      container.addChild(new Text("     " + theme.fg("dim", padR("标题", titleW)) + " " + theme.fg("dim", padR("状态", statusW)) + " " + theme.fg("dim", padR("优先", priorityW)) + (kind === "bug" ? " " + theme.fg("dim", padR("严重度", severityW)) : "") + " " + theme.fg("dim", padR("开始", dateW)) + " " + theme.fg("dim", padR("结束", dateW)), 1, 0));
      for (const line of treeList.render(curW, theme)) container.addChild(new Text(line, 1, 0));
      if (kind === "bug") {
        const selected = treeList.getSelectedItem();
        if (selected) container.addChild(new Text(theme.fg("muted", `当前 Bug：${oneLine(selected.name)}`), 1, 0));
      }
      ...
      const hint = focusSearch
        ...
        : searching
          ...
          : "↑↓ 导航  Space/→/← 展开收起  Enter 关联会话  o 浏览器打开  / 搜索  Tab 切换需求/Bug  i 切换迭代" + (kind === "story" ? "  t 类型" : "") + "  Esc/Ctrl+C 退出";
      ...
      handleInput(data: string) {
        if (focusSearch) {
          ...
          if (data === "\r" || data === "\n") {
            ...
            if (it) done({ action: "link_view", itemKey: linkKey(it.workspaceId, it.id, it.kind), itemName: it.name });
            return;
          }
        ...
        if (data === "\t" || data === "\x1b[Z") { done({ action: "kind_toggle" }); return; }
        if (data === "i") { done({ action: "scope_toggle" }); return; }
        if (data === "t" && kind === "story") { done({ action: "type_filter" }); return; }
        if (data === "\r" || data === "\n") {
          const it = treeList.getSelectedItem();
          if (it) done({ action: "link_view", itemKey: linkKey(it.workspaceId, it.id, it.kind), itemName: it.name });
          return;
